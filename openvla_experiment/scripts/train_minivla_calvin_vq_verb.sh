#!/bin/bash
#SBATCH --job-name=minivla_vq_verb
#SBATCH --partition=general
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=30:00:00
#SBATCH -o /data/user_data/wenjiel2/Code/VLA-in-reverse-VA-to-L-classifier/logs/minivla_vq_verb-%j.out
#SBATCH -e /data/user_data/wenjiel2/Code/VLA-in-reverse-VA-to-L-classifier/logs/minivla_vq_verb-%j.err
...
torchrun --standalone --nnodes 1 --nproc-per-node 1 vla-scripts/train.py \
    --vla.type "prism-qwen25-dinosiglip-224px+0_5b+mx-calvin-d-vq-verb" \
    --data_root_dir "${DATA_DIR}" \
    --run_root_dir "${RUN_DIR}" \
    --image_aug True \
    --save_interval 5000 \
    --run_id_note calvin_d_vq_verb
